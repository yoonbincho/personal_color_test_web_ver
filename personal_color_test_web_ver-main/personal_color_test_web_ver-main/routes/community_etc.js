...
app.post("/insert", (req, res) => {
  ...
  if (title == "" || content == "") {
    return res.send(
      "<script>alert('제목과 내용을 입력해주세요!'); location.href='/community/etc/insert';</script>"
    );
    ...
    let sql =
      "insert into etc(user_id, title, date, content, img) values ('" +
      req.session.user.id +
      "', '" +
      title +
      "', now(), '" +
    ...
    return res.send(
      "<script>alert('작성이 완료되었습니다.'); location.href='/community/etc/';</script>"
    );
  }
});
...
app.get("/", (req, res) => {
  mainsql =
    "select id, user_id, title, date_format(date,'%Y-%m-%d %H:%i:%s') date, content, img from etc order by id desc";

  res.redirect("/community/etc/1");
});
...
app.post("/", (req, res) => {
  ...
  if (search == "user" && sw != "") {
    mainsql =
      "select id, user_id, title, date_format(date,'%Y-%m-%d %H:%i:%s') date, content, img  from etc where user_id like '%" +
      sw +
      "%' order by id desc";
  } else if (search == "title" && sw != "") {
    mainsql =
      "select id, user_id, title, date_format(date,'%Y-%m-%d %H:%i:%s') date, content, img  from etc where title like '%" +
      sw +
      "%' order by id desc";
  } else {
    mainsql =
...
app.get("/delete/:id", function (req, res, next) {
  ...
  let sql = "delete from etc where id = " + id;
  conn.query(sql, id, function (err, row) {
    if (err) console.error(err);
  });

  res.send(
    "<script>alert('삭제가 완료되었습니다. '); location.href='/community/etc';</script>"
  );
});
